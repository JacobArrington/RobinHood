  ...
  return (
    <div className="owned-stock-container">
      {Object.values(userOwnedStocks).map((stock) => {
        if (stock.totalShares > 0) {
          return (
            <div className="owned-stock-card" key={stock?.stockName}>
              <h3 className="stock-name">Stock Name: {stock?.stockName}</h3>
              <p className="total-shares">Total Shares Owned: {stock.totalShares}</p>
              <p className="total-value">Total Value: ${stock.totalValue.toFixed(2)}</p>
            </div>
          );
        }
        return null;
